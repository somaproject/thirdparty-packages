...
public:
  ...
  Fast_convolution(const_Vector<value_type, B> weights)
    : weights_(t2f(weights)),
      for_fftm_(Domain<2>(weights.size(), weights.size()), 1.),
      inv_fftm_(Domain<2>(weights.size(), weights.size()), 1./weights.size())
  {}
...
private:
  vsip::impl::cbe::Fastconv<1, value_type, vsip::impl::dense_complex_type> fastconv_;
};
...
private:
  vsip::impl::cbe::Fastconv<2, value_type, vsip::impl::dense_complex_type> fastconv_;
};
#endif
