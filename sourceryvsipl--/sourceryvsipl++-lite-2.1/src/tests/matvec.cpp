...
Test_gemp( T alpha, T beta, length_type M, length_type P, length_type N )
{
  ...
  for ( row = 0; row < M; ++row )
    for ( col = 0; col < N; ++col )
    {
      T dot = 0;
      for ( index_type i = 0; i < P; ++i )
        dot += vsip::impl::impl_conj(a_t.get(i, row)) * b_t.get(col, i);
      d.put( row, col, alpha * dot + beta * c(row, col) );
    }
  ...
  for ( row = 0; row < M; ++row )
    for ( col = 0; col < N; ++col )
    {
      T dot = 0;
      for ( index_type i = 0; i < P; ++i )
        dot += a.get(row, i) * vsip::impl::impl_conj(b.get(i, col));
      d.put( row, col, alpha * dot + beta * c(row, col) );
    }
...
Test_gems( T alpha, T beta, length_type M, length_type /*P*/, length_type N )
{
  ...
  for ( index_type row = 0; row < M; ++row )
    for ( index_type col = 0; col < N; ++col )
      d.put( row, col, alpha * vsip::impl::impl_conj(a_t.get(col, row)) + beta * c(row, col) );
  ...
  for ( index_type row = 0; row < M; ++row )
    for ( index_type col = 0; col < N; ++col )
      d.put( row, col, alpha * vsip::impl::impl_conj(a.get(row, col)) + beta * c(row, col) );
