...
template <typename T,
	  typename Block>
inline
const_Vector<T, const vsip::impl::Unary_expr_block<1, vsip::impl::op::Minus, Block, T> >
t_neg(
  const_Vector<T, Block> v1)
{
  typedef vsip::impl::Unary_expr_block<1, vsip::impl::op::Minus, Block, T> block_t;
...
const_Vector<typename Promotion<T1, T2>::type,
	     const vsip::impl::Binary_expr_block<1, vsip::impl::op::Add, 
					 Block1, typename Promotion<T1, T2>::type,
					 Block2, typename Promotion<T1, T2>::type> >
t_add(
  const_Vector<T1, Block1> v1,
  const_Vector<T2, Block2> v2)
{
  typedef typename Promotion<T1, T2>::type RT;
  typedef vsip::impl::Binary_expr_block<1, vsip::impl::op::Add, 
                                Block1, typename Promotion<T1, T2>::type,
                                Block2, typename Promotion<T1, T2>::type> block_t;
...
inline
const_Vector<typename Promotion<T1, T2>::type,
	     const vsip::impl::Binary_expr_block<1, vsip::impl::op::Mult,
					 Block1, typename Promotion<T1, T2>::type,
					 Block2, typename Promotion<T1, T2>::type> >
t_mul(
  const_Vector<T1, Block1> v1,
...
{
  typedef typename Promotion<T1, T2>::type RT;
  typedef vsip::impl::Binary_expr_block<1, vsip::impl::op::Mult,
                                Block1, typename Promotion<T1, T2>::type,
                                Block2, typename Promotion<T1, T2>::type> block_t;
...
template <dimension_type            D,
	  ...
	  typename                  Block,
	  typename                  Type>
struct Block_name<impl::Unary_expr_block<D, Operator, Block, Type> >
{
  static string name() { return string("Unary_expr_block<")
...
template <dimension_type                      D,
	  ...
	  typename                            RBlock,
	  typename                            RType>
struct Block_name<impl::Binary_expr_block<D, Operator, 
				       LBlock, LType,
				       RBlock, RType> >
{
  static string name() { return string("Binary_expr_block<")
...
test_funcall()
{
  ...
  const_Vector<T, const vsip::impl::Binary_expr_block<1, vsip::impl::op::Add,
                                              Dense<1, T>, T, 
                                              Dense<1, T>, T> > vx(t_add(v1, v2));
  T s3 = sum(vx);
