...
template <typename VT1, typename BT1,
	  typename VT2, typename BT2>
void
add_numeric_test(BT1 p, BT2 q)
{
  typedef typename Promotion<VT1, VT2>::type VT;
  typedef Vector<VT, Dense<1, VT> > Result;
  typedef Dense<1, BT1> Block1;
  typedef Dense<1, BT2> Block2;
  typedef const_Vector<VT1, Block1> View1;
  typedef const_Vector<VT2, Block2> View2;
  Domain<1> domain(1);
  Block1 b1(domain, p);
  Block2 b2(domain, q);
...
main(int argc, char** argv)
{
  ...
  add_numeric_test<float, float, float, float>(0.6, 0.6);

  // float blocks in int views: (int)0.6 + (int)0.6 == 0
  add_numeric_test<int, float, int, float>(0.6, 0.6);

  // Add one int view to one float view: (int)0.6 + 0.6 == 0.6
  add_numeric_test<int, float, float, float>(0.6, 0.6);
