...
{
  vsip::impl::data_access::Low_level_data_access<vsip::impl::Direct_access_tag,
                                        Dense<1, float>,
...
// Sum values in a dense block via Ext_data interface.

float
ext_sum(Dense<1, float>& block)
{
  vsip::impl::Ext_data<Dense<1, float> > raw(block, vsip::impl::SYNC_IN);
  float*      data   = raw.data();
...
// Sum values in a block via Ext_data interface.

/// For Dense, this should use data_interface::Direct_data interface
/// For Plain_block, this should use data_interface::Copy_data interface
...
gen_ext_sum(Block& block)
{
  ...
  vsip::impl::Ext_data<Block> raw(block, vsip::impl::SYNC_IN);

  value_type* data   = raw.data();
  stride_type stride = raw.stride(0);
...
// Sum values in a view using Ext_data-interface.
...
{
  vsip::impl::Ext_data<Block> raw(view.block());
  float*   data   = raw.data();
  stride_type stride = raw.stride(0);
...
{
  vsip::impl::Ext_data<Block1> raw_res(res.block(), vsip::impl::SYNC_OUT);
  float*   p_raw   = raw_res.data();
  stride_type str_raw = raw_res.stride(0);

  vsip::impl::Ext_data<Block2> raw1(op1.block(), vsip::impl::SYNC_IN);
  float*   p1   = raw1.data();
  stride_type str1 = raw1.stride(0);

  vsip::impl::Ext_data<Block3> raw2(op2.block(), vsip::impl::SYNC_IN);
  float*   p2   = raw2.data();
...
// Dot-product of two views using Ext_data interface.
  ...
  const_Vector<T2, Block2> op2)
{
  ...
  vsip::impl::Ext_data<Block1> raw1(op1.block());
  T1*      p1   = raw1.data();
  stride_type str1 = raw1.stride(0);

  vsip::impl::Ext_data<Block2> raw2(op2.block());
  T2*      p2   = raw2.data();
  stride_type str2 = raw2.stride(0);
...
test_block_sum()
{
  ...
  Plain_block<1, float> pb(size, 0.f);
...
test_1_low()
{
  ...
  {
    vsip::impl::data_access::Low_level_data_access<AccessTag, Block,
...
test_1_ext()
{
  ...
  {
    vsip::impl::Ext_data<Block> raw(block);
...
test_dense_2()
{
  ...
  vsip::impl::Ext_data<Row_major_block> row_raw(row_blk);
  vsip::impl::Ext_data<Col_major_block> col_raw(col_blk);
...
test_dense_12()
{
  ...
  using vsip::impl::No_count_policy;
  ...
  {
    vsip::impl::Ext_data<Block, Layout<2, Order,
  ...
  {
    vsip::impl::Ext_data<Block, Layout<1, row1_type,
      typename Block::pack_type,
      typename Block::complex_type>, RP> raw(block, SYNC_IN);
...
main(int argc, char** argv)
{
  ...
  // test_1_low<Plain_block<1, float>, vsip::impl::Ref_count_policy, Direct_data>();
  test_1_low<Plain_block<1, float>, Copy_access_tag>();

  // Test high-level data interface.
  test_1_ext<Dense<1,      float>, vsip::impl::Ref_count_policy>();
  test_1_ext<Dense<1,      float>, vsip::impl::No_count_policy>();
  test_1_ext<Plain_block<1, float>, vsip::impl::Ref_count_policy>();
  test_1_ext<Plain_block<1, float>, vsip::impl::No_count_policy>();
