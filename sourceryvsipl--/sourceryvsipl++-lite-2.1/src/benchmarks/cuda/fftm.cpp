...
*/
/** @file    benchmarks/cuda/fftm.cpp
    @author  Don McCoy
    @date    2009-04-01
    @brief   VSIPL++ Library: Benchmark for Fast Fourier Transforms using CUDA
*/
...
struct t_fftm<std::complex<float>, Impl_op> : Benchmark_base
{
  ...
  void fftm(length_type rows, length_type cols, length_type loop, float& time)
  {
    ...
    {
      vsip::impl::Ext_data<block_type> ext_in(data_in.block());
      vsip::impl::Ext_data<block_type> ext_out(data_out.block());
      T* p_in = ext_in.data();
      T* p_out = ext_out.data();
...
struct t_fftm<std::complex<float>, Impl_ip> : Benchmark_base
{
  ...
  void fftm(length_type rows, length_type cols, length_type loop, float& time)
  {
    ...
    {
      vsip::impl::Ext_data<block_type> ext_data(data.block());
