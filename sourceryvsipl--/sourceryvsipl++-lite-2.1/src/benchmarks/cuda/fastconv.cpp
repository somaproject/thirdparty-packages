...
*/
/** @file    benchmarks/cuda/fastconv.cpp
    @author  Don McCoy
    @date    2009-03-11
    @brief   VSIPL++ Library: Benchmark for Fast Convolution using CUDA.
*/
...
struct t_fastconv_base<T, Impl_mem_phased> : fastconv_ops
{
		...
		length_type loop, float& time)
  {
    ...
    {
      vsip::impl::Ext_data<mat_block_type> ext_data(data.block());
      vsip::impl::Ext_data<vec_block_type> ext_replica(replica.block());
      T* p_data = ext_data.data();
      T* p_replica = ext_replica.data();
      ...
      for (index_type l=0; l<loop; ++l)
      {
        ...
        vsip::impl::cuda::vmmuls_row_cc(
          d_replica, d_data, d_data, 1 / (float)nrange, npulse, nrange);
...

    {
      ...
      for (index_type l=0; l<loop; ++l)
      {
        ...
        for (length_type r = 0; r < npulse; ++r)
        {
          ...
          vsip::impl::cuda::vmmuls_row_cc(
            d_replica, &d_data[row], &d_data[row], 1 / (float)nrange, 1, nrange);
