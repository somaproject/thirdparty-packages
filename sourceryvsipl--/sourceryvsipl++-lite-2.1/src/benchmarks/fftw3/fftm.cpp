...
/** @file    benchmarks/fftm_fftw3.cpp
    @author  Jules Bergmann
    @date    2006-10-19
    @brief   VSIPL++ Library: Benchmark for FFTW3 FFTM.
*/
...
struct t_fftm<complex<float>, Cmplx_inter_fmt, Impl_op, OrderT>
  : Benchmark_base
{
  ...
  {
    typedef impl::Layout<2, OrderT, Stride_unit_dense, ComplexFmt> LP;
    typedef impl::Fast_block<2, T, LP, Local_map> block_type;
	...
	fftwf_forget_wisdom();
      ...
      p = fftwf_plan_many_dft(
		...
		n,				// n - dim size
		howmany,			// homany
		(fftwf_complex*)(ext_A.data()), // in
		NULL,				// inembed
		istride,			// istride
		idist,				// idist
		(fftwf_complex*)(ext_Z.data()), // out
		NULL,				// onembed
		ostride,			// ostride
		odist,				// odist
		FFTW_FORWARD,			// sign
...
template <typename T, typename CmplxFmt, typename ImplTag, typename OrderT>
struct t_fftm_fix_rows : public t_fftm<T, CmplxFmt, ImplTag, OrderT>
{
  typedef t_fftm<T, CmplxFmt, ImplTag, OrderT> base_type;
...
template <typename T, typename CmplxFmt, typename ImplTag, typename OrderT>
struct t_fftm_fix_cols : public t_fftm<T, CmplxFmt, ImplTag, OrderT>
{
  typedef t_fftm<T, CmplxFmt, ImplTag, OrderT> base_type;
  static int const elem_per_point = base_type::elem_per_point;
