...
*/
/** @file    benchmarks/sort.cpp
    @author  Mike LeBlanc
    @date    2009-04-27
    @brief   VSIPL++ Library: Benchmark for sort.
...
struct t_std_sort : Benchmark_base
{
  ...
  typedef
    typename vsip::impl::ITE_Type<
      ascend,
      vsip::impl::As_type<std::less<T> >,
      vsip::impl::As_type<std::greater<T> >
      >::type
    comp_type;
    ...
    VSIP_IMPL_NOINLINE
  {
    ...
    vsip::impl::Ext_data<typename Vector<T>::block_type, layout_type>
      raw(A.block(), vsip::impl::SYNC_INOUT);
