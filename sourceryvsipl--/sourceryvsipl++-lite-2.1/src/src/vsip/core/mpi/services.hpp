...
/** @file    vsip/core/parallel/mpi.hpp
    @author  Jules Bergmann
    @date    2005-03-25
    @brief   VSIPL++ Library: Parallel Services: MPI
...
public:
  Par_service(int& argc, char**& argv)
    : valid_(MPI_Init(&argc, &argv) == 0 ? 1 : 0),
      buf_  (new char[BUFSIZE])
    {
      assert(valid_);

      default_communicator_ = communicator_type(MPI_COMM_WORLD);
      MPI_Buffer_attach(buf_, BUFSIZE);

      // Initialize complex datatypes.
      mpi::Mpi_datatype<std::complex<float> >::value();
      mpi::Mpi_datatype<std::complex<double> >::value();
    }
