...
struct Profile_nop_policy<
  Fir_tag, 
  Ref_counted_ptr<Fir_backend<T, S, C> >(aligned_array<T>,
                                         length_type,
                                         length_type,
                                         length_type,
                                         unsigned,
  ...
  B>
{
  Profile_nop_policy(aligned_array<T> const &,
                     length_type,
                     length_type,
                     length_type,
                     unsigned,
...
struct Profile_policy<
  ...
  B>
{
  typedef profile::Scope<Profile_feature<Fir_tag>::value> scope_type;
  typedef Evaluator<Fir_tag, B,
    Ref_counted_ptr<Fir_backend<T, S, C> >(aligned_array<T>,
                                           length_type,
                                           length_type,
                                           length_type,
  ...
  Profile_policy(aligned_array<T> const &k,
                 length_type ks,
                 length_type is,
                 length_type d,
                 unsigned n,
    ...
    : accumulator_type(impl::signal_detail::Description<1, T>::tag(
      ...
      backend_(new impl::cvsip::Fir_impl<T, S, C>
               (copy(kernel.block()), kernel.size(), input_size, decimation,
                N, H))
#else
      backend_(impl::dispatch<impl::Fir_tag, backend_ptr>
               (copy(kernel.block()), kernel.size(), input_size, decimation,
                N, H))
#endif
  {}
