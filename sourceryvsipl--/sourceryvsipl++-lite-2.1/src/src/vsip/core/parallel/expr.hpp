...
/** @file    vsip/core/parallel/expr.hpp
    @author  Jules Bergmann
    @date    2005-03-28
    @brief   VSIPL++ Library: Chained parallel assignment algorithm.
...
public:
  Par_expr(
    typename View_of_dim<Dim, value1_type, DstBlock>::type       dst,
    typename View_of_dim<Dim, value2_type, SrcBlock>::const_type src)
  : dst_      (dst),
  ...
  void operator()()
  {
    typedef typename View_of_dim<Dim, value1_type, dst_lblock_type>::type
		dst_lview_type;
    typedef typename View_of_dim<Dim, value2_type, src_lblock_type>::const_type
		src_lview_type;
  ...
  // Member data.
private:
  typename View_of_dim<Dim, value1_type, DstBlock>::type           dst_;
  typename View_of_dim<Dim, value2_type, src_peb_type>::const_type src_remap_;
};
...
// Requires:
//   DST is a distributed, non-const view to assign to.
//   SRC is a distributed view of the same dimension as DST that holds
