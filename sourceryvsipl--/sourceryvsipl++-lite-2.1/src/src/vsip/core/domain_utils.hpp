...
/** @file    vsip/core/domain_utils.hpp
    @author  Jules Bergmann
    @date    2005-03-22
    @brief   VSIPL++ Library: Domain utilities.
...
// Specialization for projecting from a 1-dimensional domain.

template <>
struct Project_domain<1>
{
  template <dimension_type Dim1>
  static Domain<1> project(Domain<Dim1> const& dom)
  {
    assert(1 <= Dim1);
...
// Specialization for projecting from a 2-dimensional domain.

template <>
struct Project_domain<2>
{
  template <dimension_type Dim1>
  static Domain<2> project(Domain<Dim1> const& dom)
  {
    assert(2 <= Dim1);
...
// Specialization for projecting from a 3-dimensional domain.

template <>
struct Project_domain<3>
{
  template <dimension_type Dim1>
  static Domain<3> project(Domain<Dim1> const& dom)
  {
    assert(3 <= Dim1);
    return dom;
  }
...
// Project a ORIGDIM-dimension domain to PROJDIM dimensions.
// (where PROJDIM <= ORIGDIM)

template <dimension_type ProjDim,
	  dimension_type OrigDim>
Domain<ProjDim>
project(
  Domain<OrigDim> const& dom)
{
...
// Construct a Domain<Dim> from an array of Dim Domain<1>s.
...
//      Interstection is placed in RES and TRUE is returned,
//   Otherwise,
//      RES is unchanged and FALSE is returned.
...
bool
intersect(
  Domain<Dim> const& dom1,
  Domain<Dim> const& dom2,
  Domain<Dim>&       res)
{
...
{
  return domain_utils::Block_domain_class<Dim, BlockT>::func(block);
}
...
#endif // VSIP_CORE_DOMAIN_UTILS_HPP
