  ...
  unsigned int      iter_max)
{
  ...
  float* in  = (float*)inout + 0      * size*2;
  float* out = (float*)inout + chunks * size*2;

  for (i=0; i<chunks; ++i)
  {
    float* t_in  = in  + i*size*2;
    float* t_out = out + i*size*2;

    cml_ccfft1d_op_f(obj, t_in, t_out, CML_FFT_FWD, buf);

    if (fftp->direction == fwd_fft)
    {
      if (fftp->scale != (double)1.f)
	cml_core_rcsvmul1_f(fftp->scale, t_out, t_out, size);
    }
    else
    {
      ...
      vector float* start = (vector float*)t_out;
  ...
  unsigned int      total_count)
{
  ...
  case overlay_cvmmul_row_f:
    return input_cvmmul_row_f(pf, p_context, p_params, entries,
			      current_count, total_count);
  case overlay_cfft_f:
    return input_cfft_f(pf, p_context, p_params, entries,
			current_count, total_count);
  }
  return 1;
}
  ...
  unsigned int      total_count)
{
  ...
  {
  case overlay_cvmmul_row_f:
    return output_cvmmul_row_f(pf, p_context, p_params, entries,
			       current_count, total_count);
  case overlay_cfft_f:
    return output_cfft_f(pf, p_context, p_params, entries,
			 current_count, total_count);
  }
  return 1;
}
  ...
  unsigned int      iter_count)
{
  ...
  {
  case overlay_cvmmul_row_f:
    return kernel_cvmmul_row_f(pf, p_context, p_params, inout,
			       iter, iter_count);
  case overlay_cfft_f:
    return kernel_cfft_f(pf, p_context, p_params, inout,
			 iter, iter_count);
  }
  return 1;
}
