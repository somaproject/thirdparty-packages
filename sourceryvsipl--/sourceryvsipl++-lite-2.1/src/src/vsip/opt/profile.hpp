...
#if (VSIP_IMPL_PROFILE_TIMER == 1 || VSIPL_IMPL_PROFILE_TIMER == 2)
// Posix_time uses clock()
// Posix_real_time uses clock_gettime()
#  include <time.h>
#endif
...
typedef Posix_real_time  DefaultTime;
...
public:
  ...
  int event(std::string const &name, int value, int id, 
