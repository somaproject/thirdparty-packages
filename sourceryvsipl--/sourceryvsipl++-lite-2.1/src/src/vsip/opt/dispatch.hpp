...
{
private:
  typedef typename Make_type_list<T2, T3, T4, T5, T6, T7, T8, T9>::type Rest;
public:
...
/// e.g 'Dispatcher<Operation, float>::backend'.
template <typename O,
          typename T,
...
{
  typedef B backend;
  typedef typename E::backend_type backend_type;
};
...
/// FIXME: the original code doesn't have this specialization. Why does it work ?
template <typename O, typename R, typename A, typename L, typename B, typename E>
