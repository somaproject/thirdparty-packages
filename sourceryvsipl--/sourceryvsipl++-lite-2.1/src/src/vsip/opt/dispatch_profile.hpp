...
{
  static unsigned int const value = profile::none;
};
...
{
  typedef profile::Scope<Profile_feature<O>::value> scope_type;
...
struct Profile_policy_selector
{
  typedef typename ITE_Type<
    Profile_feature<O>::value & profile::mask,
    As_type<Profile_policy<O, S, B> >,
    As_type<Profile_nop_policy<O, S, B> > >::type type;
};
