...
template <typename T>
struct Is_svd_impl_avail<Lapack_tag, T>
{
...
template <typename T,
	  bool     Blocked>
class Svd_impl<T,Blocked,Lapack_tag>
{
...
template <typename T,
	  bool     Blocked>
Svd_impl<T, Blocked,Lapack_tag>::Svd_impl(
  length_type  rows,
  length_type  cols,
  storage_type ust,
  storage_type vst
...
template <typename T,
	  bool     Blocked>
Svd_impl<T, Blocked,Lapack_tag>::Svd_impl(Svd_impl const& sv)
VSIP_THROW((std::bad_alloc))
  : m_          (sv.m_),
    n_          (sv.n_),
    p_          (sv.p_),
...
template <typename Block0,
	  typename Block1>
bool
Svd_impl<T, Blocked,Lapack_tag>::impl_decompose(
  Matrix<T, Block0>              m,
...
template <mat_op_type       tr,
	  ...
	  typename          Block1>
bool
Svd_impl<T, Blocked,Lapack_tag>::impl_produ(
...
template <mat_op_type       tr,
	  ...
	  typename          Block1>
bool
Svd_impl<T, Blocked, Lapack_tag>::impl_prodv(
  const_Matrix<T, Block0> b,
  Matrix<T, Block1>       x)
  VSIP_NOTHROW
{
...
template <typename Block>
bool
Svd_impl<T, Blocked,Lapack_tag>::impl_u(
  index_type       low,
  index_type       high,
  Matrix<T, Block> u)
  VSIP_NOTHROW
...
template <typename Block>
bool
Svd_impl<T, Blocked,Lapack_tag>::impl_v(
  index_type       low,
  index_type       high,
  Matrix<T, Block> v)
  VSIP_NOTHROW
