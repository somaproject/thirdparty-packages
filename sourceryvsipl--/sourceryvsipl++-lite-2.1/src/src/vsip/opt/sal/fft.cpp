...
//     pack(in, size_[axis] + 2, size_[1 - axis]);
    pack(in, size_[axis] + 2, size_[1 - axis]);
//     std::cout << in_stride << ' ' << out_stride << std::endl;
...
template <typename T, int A, int E>
class Fft_impl<2, std::complex<T>, std::complex<T>, A, E>
...
public:
			    ...
			    length_type rows, length_type cols)
  {
    ...
    if (A == 0)
    {
      ...
      cop2d(in, in_c_stride, out, out_c_stride, A, direction);
      if (!almost_equal(this->scale_, T(1.)))
	for (length_type i = 0; i != cols; ++i)
    ...
    else
    {
      ...
      cop2d(in, in_r_stride, out, out_r_stride, A, direction);
      if (!almost_equal(this->scale_, T(1.)))
	for (length_type i = 0; i != rows; ++i)
...
template <typename T, int A>
class Fft_impl<2, T, std::complex<T>, A, -1>
  : private Fft_base<2, precision<T>::single>,
...
template <typename T, int A>
class Fft_impl<2, std::complex<T>, T, A, 1>
  : private Fft_base<2, precision<T>::single>,
...
public:
			    ...
			    length_type rows, length_type cols)
  {
    ...
    {
      assert(in_r_stride == 1 && out_r_stride == 1);
      ropm(in, in_c_stride, out, out_c_stride, A);
      // Scaling done in ropm()
    }
    else // A == 1
    {
      assert(in_c_stride == 1 && out_c_stride == 1);
      ropm(in, in_r_stride, out, out_r_stride, A);
      // Scaling done in ropm()
    }
  }
  virtual void by_reference(std::pair<T *, T *>,
