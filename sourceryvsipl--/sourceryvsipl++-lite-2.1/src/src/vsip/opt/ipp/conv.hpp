...
protected:
  ...
  typedef Layout<dim, typename Row_major<dim>::type, Stride_unit, Cmplx_inter_fmt>
    layout_type;
...
VSIP_NOTHROW
{
  ...
  {
    if (decimation_ == 1 && in_ext.stride(0) == 1 && out_ext.stride(0) == 1)
    {
  ...
  {
    if (decimation_ == 1 && in_ext.stride(0) == 1)
    {
...
template <symmetry_type       S,
	  ...
	  unsigned            N,
          alg_hint_type       H>
struct Evaluator<Conv_tag<1, S, R, float, N, H>, Intel_ipp_tag>
{
  static bool const ct_valid = true;
  typedef ipp::Convolution<const_Vector, S, R, float, N, H> backend_type;
};
template <symmetry_type       S,
	  support_region_type R,
	  unsigned            N,
          alg_hint_type       H>
struct Evaluator<Conv_tag<1, S, R, double, N, H>, Intel_ipp_tag>
{
  static bool const ct_valid = true;
  typedef ipp::Convolution<const_Vector, S, R, double, N, H> backend_type;
};
template <symmetry_type       S,
	  support_region_type R,
	  unsigned            N,
          alg_hint_type       H>
struct Evaluator<Conv_tag<2, S, R, short, N, H>, Intel_ipp_tag>
{
  static bool const ct_valid = true;
  typedef ipp::Convolution<const_Matrix, S, R, short, N, H> backend_type;
};
template <symmetry_type       S,
	  support_region_type R,
	  unsigned            N,
          alg_hint_type       H>
struct Evaluator<Conv_tag<2, S, R, float, N, H>, Intel_ipp_tag>
{
  static bool const ct_valid = true;
  typedef ipp::Convolution<const_Matrix, S, R, float, N, H> backend_type;
};
